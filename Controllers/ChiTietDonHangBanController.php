...
class ChiTietDonHangBanController{
    ...
    public function DanhSachTQ() {
        if (isset($_GET['id'])) {
            $id = $_GET['id'];
            $info = $this->donhangban->ChiTietTQ($id);
            $list_details = $this->model->DanhSach1($info[0]['ID']);
        }
        include("Views/ChiTietDonHangBan/DanhSachTQ.php");
    }
    ...
    public function DanhSach(){
        ...
        $danhsachdonhang = $this->donhangban->DanhSachTrangThai();
        if (isset($_GET['id'])) {
            $id = $_GET['id'];
            $table = 'donhangban';
            ...
            if(isset($_POST['button1'])) {
                ...
                for ($i = 0; $i < count($result11); $i++) {
                    ...
                    if ($result11[$i]['SoLuong'] > $this->model->SoLuongSanPham($result11[$i]['idSanPham'])[0]['SoLuong']) {
                        $co = 0;
                        break;                   
                    }  
                }
                if($co === 1) {
                    for ($i=0;$i<count($result11);$i++){
                        $this->model->hihi($result11[$i]['idSanPham'],$result11[$i]['SoLuong']);
                    }
                    $this->donhangban->CapNhatTrangThaiHoanThanhDonHang($id);
                    $_SESSION['hehe'] = "true";
                 } else {
            ...
            if(isset($_POST['updateStatus'])) {
                ...
                $tentrangthai = $this->donhangban->tentrangthai($_POST['status']);
                $tentrangthai1 = $tentrangthai[0]['TenTrangThai'];
                $content = 'Đơn hàng của bạn đang trong trạng thái ' .$tentrangthai1;
                $this->donhangban->CapNhatTenTrangThai($id, $_POST['status']);
            //    $sqlupdateThongBao = "UPDATE thongbao set content = 'Đơn hàng của bạn đang trong trạng thái $tentrangthai1' where ID_DH = $id";
                // $this->db->conn->query($sqlupdateThongBao);
                $LayIDKH = $this->donhangban->LayID_KH($_GET['id']);
                $this->thongbao->ThemMoi1($_GET['id'], $LayIDKH[0]['idKhachHang'], $content, date("Y-m-d H:i:s"), 0);
               header("Location: ../DonHangBan/DanhSach");
            }
        }
