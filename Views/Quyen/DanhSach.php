    ...
    <div class="row">
        ...
        <form action="" method="get">
            <div class="col-sm-12">
                <div class="row">
                    <div class="col-sm-3">
                        <input type="text" value="<?php if(isset($_GET['keyword'])) {echo $_GET['keyword'];} ?>" name="keyword" class="form-control" placeholder="Nhập ID quyền, tên quyền, path quyền,...">
                    </div>
                    <div class="col-sm-3">
                        <select name="id_nq" class="form-select" id="">
                            <option value="">Lọc theo nhóm quyền</option>
                            <?php foreach($list_LoaiQuyen as $item) { ?>
                                <option <?php if (isset($_GET['id_nq']) && $_GET['id_nq'] == $item['ID']) { echo "selected"; } ?> value="<?=$item['ID'];?>"><?=$item['tenquyen'];?></option>
                            <?php } ?>
                        </select>
                    </div>
...
<div class="col-md-12 mt-3">
    <form action="" method="POST">
        <div class="row">
            ...
            <?php if(check('Quyen/ThemMoi') == true) { ?>
    ...
    <table class="table table-condensed table-bordered text-center">
        ...
        if(!empty($result)) {
            ...
            <tr>
                ...
                <td>
                    <?php if(check('Quyen/CapNhat&id=123')) { ?>
                    <a style="margin: 0px 7px 0px 7px;padding: 7px 5px; font-size: 14px; background: green; color: #fff; border-radius: 5px;" href="../Quyen/CapNhat&id=<?=$row['ID']?>">Cập nhật</a>
                    <?php } ?>

                    <?php if(check('Quyen/Xoa&id=123')) {?>
                    <a style="padding: 7px 5px; font-size: 14px; background: red; color: #fff; border-radius: 5px;" href="../Quyen/Xoa&id=<?=$row['ID']?>" onclick="return confirm('Xác nhận xóa !');">Xóa</a>
                    <?php }?>
                </td>
            </tr>
        <?php endforeach; } else { ?>
