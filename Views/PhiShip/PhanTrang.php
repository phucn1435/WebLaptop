<div class="container">
    ...
    $url = http_build_query($filteredParams, '', '&', PHP_QUERY_RFC3986);
            ...
            $first = 1; 
        ?>
        <a class="paging-link paging-link-first " href="?<?php echo isset($_GET['tinh']) ? "$url" : ""; ?>&per_page=<?=$item1;?>&page1=<?=$first?>">First</a>
        <?php }
        if($current > 1) {
            $prev = $current - 1; ?>
        <a class="paging-link" href="?<?php echo isset($_GET['tinh']) ? "$url" : ""; ?>&per_page=<?=$item1;?>&page=<?=$prev?>">Prev</a>
    <?php } ?>
    ...
    <?php
        ...
        <a class="paging-link" href="?<?php echo isset($_GET['tinh']) ? "$url" : ""; ?>&per_page=<?=$item1;?>&page=<?=$num?>"><?=$num?></a>
        <?php } ?>
        <?php } else { ?>
    ...
    <?php 
            ...
            $next = $current + 1;
        ?>
            <a class="paging-link" href="?<?php echo isset($_GET['tinh']) ? "$url" : ""; ?>&per_page=<?=$item1;?>&page=<?=$next?>">Next</a>
        <?php } 
        if($current < $totalPage - 3) {
            $end = $totalPage; ?>
            <a class="paging-link paging-link-last" href="?<?php echo isset($_GET['tinh']) ? "$url" : ""; ?>&per_page=<?=$item1;?>&page=<?=$end?>">Last</a>
    <?php } ?>
</div>
