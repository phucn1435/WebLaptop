...
class Quyen{
    ...
    public function LocTest($keyword,$nhomquyen,$item,$offset) {
        $sql = "SELECT q.ID,q.ID_group,tenquyen,ten,duongdan FROM quyen as q, nhomquyen as nq WHERE q.ID_group = nq.ID 
        AND ('$keyword' = '' OR q.ID = '$keyword' OR q.ten LIKE '%$keyword%' OR q.duongdan LIKE '%$keyword%')
        AND ('$nhomquyen' = ''  OR q.ID_group = '$nhomquyen') LIMIT ".$item." OFFSET ".$offset;
        $result = $this->db->select($sql);
        return $result;
    }
